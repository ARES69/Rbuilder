...
#[tauri::command] pub fn git_checkout(root: String, branch: String) -> Result<CommandResult, String> { if branch.is_empty() || branch.len() > 120 || branch.chars().any(|c| c.is_whitespace() || matches!(c, '~' | '^' | ':' | '?' | '*' | '[' | '\\')) { return Err("Некорректное имя ветки".into()); } git(&root, &["checkout", &branch]) }
#[tauri::command] pub fn git_create_branch(root: String, branch: String) -> Result<CommandResult, String> { if branch.is_empty() || branch.len() > 120 || branch.chars().any(|c| c.is_whitespace() || matches!(c, '~' | '^' | ':' | '?' | '*' | '[' | '\\')) { return Err("Некорректное имя ветки".into()); } git(&root, &["switch", "-c", &branch]) }
