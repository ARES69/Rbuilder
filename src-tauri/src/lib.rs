...
#[tauri::command]
pub fn workspace_pick(app: AppHandle) -> Option<String> { app.dialog().file().blocking_pick_folder().and_then(|p| p.into_path().ok()).map(|p| p.to_string_lossy().to_string()) }
#[tauri::command]
pub fn workspace_list_files(root: String) -> Result<Vec<FileEntry>, String> { let base = fs::canonicalize(root).map_err(|e| e.to_string())?; let mut out = Vec::new(); walk(&base, &base, &mut out)?; out.sort_by(|a,b| a.path.cmp(&b.path)); Ok(out) }
#[tauri::command]
pub fn workspace_read_file(root: String, path: String) -> Result<String, String> { let path = resolve(&root, &path, true)?; if fs::metadata(&path).map_err(|e| e.to_string())?.len() > MAX_FILE_BYTES { return Err("Файл слишком большой".into()); } fs::read_to_string(path).map_err(|_| "Файл не является текстовым".into()) }
#[tauri::command]
pub fn workspace_write_file(root: String, path: String, content: String) -> Result<(), String> { if content.len() as u64 > MAX_FILE_BYTES { return Err("Файл слишком большой".into()); } fs::write(resolve(&root, &path, false)?, content).map_err(|e| e.to_string()) }
#[tauri::command]
pub fn workspace_delete_file(root: String, path: String) -> Result<(), String> { let path = resolve(&root, &path, true)?; if path.is_dir() { return Err("Удаление папок запрещено".into()); } fs::remove_file(path).map_err(|e| e.to_string()) }
...
#[tauri::command]
pub fn git_diff(root: String) -> Result<String, String> { Ok(git(&root, &["diff", "--no-ext-diff"])?.stdout) }
#[tauri::command]
pub fn git_commit(root: String, message: String) -> Result<CommandResult, String> { let added = git(&root, &["add", "--all"])?; if added.code != 0 { return Ok(added); } git(&root, &["commit", "-m", &message]) }
#[tauri::command]
pub fn git_branches(root: String) -> Result<Vec<String>, String> { Ok(git(&root, &["for-each-ref", "--format=%(refname:short)", "refs/heads"])?.stdout.lines().map(str::to_owned).filter(|s| !s.is_empty()).collect()) }
#[tauri::command] pub fn git_checkout(root: String, branch: String) -> Result<CommandResult, String> { git(&root, &["checkout", &branch]) }
#[tauri::command] pub fn git_create_branch(root: String, branch: String) -> Result<CommandResult, String> { git(&root, &["switch", "-c", &branch]) }
#[tauri::command] pub fn git_pull(root: String) -> Result<CommandResult, String> { git(&root, &["pull"]) }
#[tauri::command] pub fn git_push(root: String) -> Result<CommandResult, String> { git(&root, &["push"]) }
#[tauri::command] pub fn git_stash(root: String) -> Result<CommandResult, String> { git(&root, &["stash", "push", "-u"]) }

#[tauri::command]
pub fn terminal_run(root: String, command: String, args: Vec<String>) -> Result<CommandResult, String> { let dir = fs::canonicalize(root).map_err(|e| e.to_string())?; let out = Command::new(command).args(args).current_dir(dir).output().map_err(|e| e.to_string())?; Ok(CommandResult { code: out.status.code().unwrap_or(-1), stdout: String::from_utf8_lossy(&out.stdout).into(), stderr: String::from_utf8_lossy(&out.stderr).into() }) }
#[tauri::command]
...
#[tauri::command]
pub fn process_stop(state: State<'_, AppState>, pid: u32) -> Result<(), String> { let mut map = state.processes.lock().map_err(|_| "Process manager недоступен")?; let mut record = map.remove(&pid).ok_or("Процесс не найден")?; record.child.kill().map_err(|e| e.to_string())?; let _ = record.child.wait(); Ok(()) }
